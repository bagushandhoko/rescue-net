...
Tests marked @known_bug are CONFIRMED LEAKS found while
writing this suite (2026-09-26), reported to the owner and deliberately not
fixed yet. When one is fixed, the test starts passing and FAILS with
"looks fixed — remove @known_bug"; remove the marker then."""
...
class TestSearchFound(FrappeTestCase):
    def setUp(self):
        self.w = make_world()
        self.report = _insert(
...
class TestShelterHouseholds(FrappeTestCase):
    def setUp(self):
        self.w = make_world()
...
class TestPhoneNumbers(FrappeTestCase):
...
class TestTransportContacts(FrappeTestCase):
    def setUp(self):
        self.w = make_world()
...
class TestPersonalContacts(FrappeTestCase):
    def setUp(self):
        self.w = make_world()
        frappe.db.set_value("RN Posko", self.w.posko_a.name, "officer_in_charge_phone", PHONE)

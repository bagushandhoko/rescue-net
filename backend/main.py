...
# ============================================================
# RESOURCE / RECOVERY ROUTE SMOKE TEST
# temporary route check before full backend
# ============================================================

@app.get("/resource-profiles")
def rn_smoke_list_resource_profiles():
    return [{"status": "resource profiles route active"}]


@app.post("/resource-profiles")
def rn_smoke_create_resource_profile(payload: dict):
    return {"status": "created-test", "payload": payload}


@app.get("/recovery-projects")
def rn_smoke_list_recovery_projects():
    return [{"status": "recovery projects route active"}]


@app.post("/recovery-projects")
def rn_smoke_create_recovery_project(payload: dict):
    return {"status": "created-test", "payload": payload}


@app.get("/recovery-project-updates")
def rn_smoke_list_recovery_project_updates():
    return [{"status": "recovery updates route active"}]


@app.post("/recovery-project-updates")
def rn_smoke_create_recovery_project_update(payload: dict):
    return {"status": "created-test", "payload": payload}

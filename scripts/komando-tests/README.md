...
Run order (container `osiun-frappe-backend`, site `osiun.localhost`):
    ...
    python3 api_e2e.py                 # 86 + phases 14-15 (functions carried by create-posko request, System Manager scheme changes), real sessions over 127.0.0.1:8095
    docker exec ... < check_notify.py  # 4 checks: WhatsApp rows in RN Notification Log (run right after api_e2e.py)
    node page_jsdom.js                 # 41 + section H (SM scheme panel); needs `npm i jsdom@22` next to it; runs the real page JS in jsdom

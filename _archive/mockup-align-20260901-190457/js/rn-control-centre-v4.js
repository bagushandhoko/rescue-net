...
function renderCriticalNeeds(ctx) {
  ...
  /*
   * Stock per item menjadi fallback.
   * Jika backend nanti menyediakan delivered /
   * realized / fulfilled quantity, field tersebut
   * akan menjadi sumber utama.
   */
  const stock = {};
  ...
  body.innerHTML =
    rows.map(row => {
       ...
       *
       * 1. realized_quantity
       * 2. fulfilled_quantity
       * 3. delivered_quantity
       * 4. quantity_fulfilled
       * 5. quantity_delivered
       * 6. stock tersedia sebagai fallback
       */
      ...
      const realization =
        explicitRealization !== undefined
          ? explicitRealization
          : stockAvailable;

      const safeRealization =
        Math.max(
          0,
          realization
        );
        ...
        <tr>
          ...
          <td>
            ...
            >
              ...
              <div
                class="
                  cc-need-progress-track
                "
              >
                <div
                  class="
                    cc-need-progress-bar
                  "
                  style="
                    width:
                    ${percent.toFixed(1)}%
...
async function loadPublicMapPoints(
  eventId
) {
  try {
    const ctx =
      await frappeCall(
        "rescue_net.api_frontend_bridge.map_context",
        {
          disaster_event:
            eventId
        }

...
async function getPublicActiveDisasters() {
  return await frappeMethod(
...
async function renderActiveDisasterSelector() {
  let rows = [];

  try {
    rows =
      await getPublicActiveDisasters();
  } catch (err) {
    console.error(
      "public active disasters failed",
      err
    );
    return;
  }
  ...
  /*
   * Selector harus menjadi bagian dari
   * Live Disaster card, bukan section baru.
   */
  const disasterName =
  ...
  const liveCard =
    disasterName.closest(
      "section, article, .panel, .card, .rn-card"
    )
    || disasterName.parentElement;
  ...
  if (!host) {
    ...
    host.className =
      "rn-active-disaster-switcher";

    /*
     * Masuk setelah metadata Live Disaster,
     * bukan sebelum seluruh dashboard.
     */
    const meta =
      document.getElementById(
        "disasterMeta"
  ...
  host.innerHTML = `
    <div class="rn-active-disaster-switcher__head">
      <span class="rn-active-disaster-switcher__label">
        Bencana Aktif
      </span>

      <span class="rn-active-disaster-switcher__count">
        ${rows.length} kejadian aktif
      </span>
    </div>

    <div class="rn-active-disaster-switcher__control">
      <select
        id="activeDisasterSelect"
        class="rn-active-disaster-select"
        aria-label="Pilih bencana aktif"
      >
        ${rows.map(row => {
          const id =
            publicEventId(row);
      ...
      <span
        class="rn-active-disaster-status"
        id="activeDisasterStatus"
      ></span>
    </div>
  `;
  ...
  const select =
    document.getElementById(
      "activeDisasterSelect"
    );

  const status =
    document.getElementById(
      "activeDisasterStatus"
    );
  ...
  function updateStatus() {
    ...
    status.textContent =
      `${row.severity || "-"} · `
      + `${row.event_status || row.status || "active"}`;
  }
...
async function loadWarRoom() {
  setText(
    "warRoomStatus",
    "Loading live AI context..."
  );

  await renderActiveDisasterSelector();
  ...
  /*
   * Control Centre adalah PUBLIC VIEWER.
   *
   * attachResourceRecovery() lama masih memakai
   * api_sync.pull yang membutuhkan login.
   * Jangan panggil endpoint private dari Guest.
   *
   * Data utama tetap berasal dari
   * api_ai.public_context.
   */
  ...
  setText(
    "severityChip",
    `Severity: ${safe(disasterSeverity)}`
  );
  ...
  try { renderSpecialPrograms(ctx); } catch (err) { console.error('render special programs safe failed', err); }
}

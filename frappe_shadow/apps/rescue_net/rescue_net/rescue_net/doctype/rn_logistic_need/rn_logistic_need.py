...
class RNLogisticNeed(Document):
    def autoname(self):
        if self.legacy_id:
            self.name = self.legacy_id
            return

        seed = f"{self.posko or ''}:{self.item_name or ''}:{frappe.generate_hash(length=12)}"
        self.name = "rn-logistic-need-" + hashlib.sha256(
            seed.encode()
        ).hexdigest()[:20]
    ...
    def before_insert(self):
        ...
        if not self.need_status:
            self.need_status = "open"
